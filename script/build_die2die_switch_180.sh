#!/usr/bin/env bash
# Usage $ build_die2die_180.sh n6 v0.0.1 11M_2X1Ya3Y2Yy2R 1P11M_2X_hv_1Ya_h_3Y_vhv_2Yy2R
...
sed -i "${_line_to_change}i\\/\/#DEFINE IO_DIE2DIE_CDM_35V_180mA   //Intel switch for Die2Die protection more info on goto/ESD" ${_path_component}/RSF_perc_esd_template

# Add path to the relaxed runset

_line_to_change=`sed -ne '/#IFDEF CDM_/=;/#IFDEF CDM_/q' ${_path_component}/RSF_perc_esd_template`
sed -i "${_line_to_change}i\#IFDEF IO_DIE2DIE_CDM_35V_180mA //add by Intel\n  INCLUDE \"./tsmc_lib_die2die/perc_${NODE_NB}.rules\"\n#ELSE\n" ${_path_component}/RSF_perc_esd_template
sed -i '$i#ENDIF //end IO_DIE2DIE_CDM_35V_180mA' ${_path_component}/RSF_perc_esd_template
