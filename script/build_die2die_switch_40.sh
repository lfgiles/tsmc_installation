#!/usr/bin/env bash
# Usage $ build_die2die_40.sh n5 v1.1.4 11M_2X1Ya3Y2Yy2R 1P11M_2X_hv_1Ya_h_3Y_vhv_2Yy2R
...
cp -R ${_path_deck}/tsmc_lib ${_path_deck}/tsmc_lib_die2die3

#`perc_*_constant.tcl` already contains the first level modification
_constant_file_relaxed=${_path_deck}/tsmc_lib_die2die3/perc_*_constant.tcl

## ESD relaxed constant values (die2die)
_constant_with_intel_relaxed_mod=script/template/constant/${NODE}/perc_${NODE_NB}_constant_die2die3.tcl
...
for file_rules in ${_path_deck}/tsmc_lib_die2die3/perc*.rules; do
    for i in "${arr_topo[@]}"
    do
...
sed -i "${_line_to_change}i\\/\/#DEFINE IO_DIE2DIE_CDM_5V_40mA   //Intel switch for Die2Die protection more info on goto/ESD" ${_path_component}/RSF_perc_esd_template

# Add path to the relaxed runset

_line_to_change=`sed -ne '/#IFDEF CDM_/=;/#IFDEF CDM_/q' ${_path_component}/RSF_perc_esd_template`
sed -i "${_line_to_change}i\#IFDEF IO_DIE2DIE_CDM_5V_40mA //add by Intel\n  INCLUDE \"./tsmc_lib_die2die3/perc_${NODE_NB}.rules\"\n#ELSE\n" ${_path_component}/RSF_perc_esd_template
sed -i '$i#ENDIF //end IO_DIE2DIE_CDM_5V_40mA' ${_path_component}/RSF_perc_esd_template
